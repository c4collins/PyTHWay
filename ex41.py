...
class CashRegister(object):
    ...
    def __init__(self):
        ...
        # Dict for building an inventory of the items and their costs from the filename passed here .csv
...
"""
    ...
    def getInventory(self, fileName):
        ...
        for row in inventoryData:
            if row[0] == 'name':
                self.invTitlesCSV = row
            else:
                self.currentInventory.append(row)
        
        for item in self.currentInventory:
            if item[3] == "FALSE":
                itemLookup[item[0]] = float(item[1])
            else:
    ...
    def addItem(self, item, quantity):
        ...
        if item not in self.items:      # checks if the item already has a price
            while True:
                try:                    # if not, it asks for a price, 
    ...
    def printReceipt(self):
        ...
        receipt = []
        receipt.append("\t\t%s" % (self.storeName))    # This is a very simple header for the receipt

        for item in self.receiptItems:          # for each item scanned
            formattedItem = "%10s  of  %s  @ %12s  is  %12s" % ("%.3f" % (item[1]),item[0].ljust(self.nameWidth)[:self.nameWidth], self.currency(self.items[item[0]]), self.currency(float(item[1])*self.items[item[0]])) 
